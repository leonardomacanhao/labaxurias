...
export class ApiService {
  ...
  getGuidesByMediumId(mediumId: string): Observable<any[]> { 
    return this.http.get<any[]>(`${this.baseUrl}/catalog/guides/medium/${mediumId}`); 
  }
  ...
  getQueueByGuide(guideId: string): Observable<any[]> { 
    return this.http.get<any[]>(`${this.baseUrl}/attendance/queue/guide/${guideId}`); 
  }

  // Calls
  callNext(guideId: string): Observable<any> { 
    return this.http.post<any>(`${this.baseUrl}/attendance/call/${guideId}`, {}); 
  }
  
  callNextBySessionEntity(sessionEntityId: string): Observable<any> { 
    return this.http.post<any>(`${this.baseUrl}/attendance/call/session-entity/${sessionEntityId}`, {}); 
  }
  
  repeatCall(queueItemId: string): Observable<any> { 
    return this.http.post<any>(`${this.baseUrl}/attendance/call/queue-item/${queueItemId}`, {}); 
  }
